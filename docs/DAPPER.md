...
dapper search [text]
dapper info <id>
dapper install <id>[@version] [--force]
...
`dapper install` writes managed apps to `/apps`. Firmware-bundled apps live in
`/system/apps` and act as fallbacks, so installing a newer copy shadows the
built-in version and removing it reveals the built-in copy again. A manually
copied `/apps/<id>.dapp` is not overwritten unless `--force` is supplied.
...
LittleFS partition. If the package registry cannot be committed, the previous app
is restored. `dapper doctor` recalculates every managed package hash and reports
missing, modified, corrupt, or runtime-incompatible installations.
