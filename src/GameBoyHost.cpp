...
// divider is exact, so pitch is true and emulator/I2S clocks barely drift.
constexpr uint32_t kSampleRate = 32768;
// One frame produces ~1098 int16 samples; sized above that so the callback
// fires once per frame from gnuboy_run's tail, not mid-frame on overflow.
constexpr size_t kSoundScratchSamples = 2048;
...
bool GameBoyHost::begin() {
  ...
  // Emulated audio flows through AudioOut when board audio came up; otherwise
  // the callback stays NULL and gnuboy mixes into the scratch silently
  // (failure-soft: a mute handheld still plays).
  if (gnuboy_init(kSampleRate, GB_AUDIO_STEREO_S16, GB_PIXEL_565_LE, nullptr,
                  AudioOut::available() ? &audioTrampoline : nullptr) != 0) {
